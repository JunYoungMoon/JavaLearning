...
public class Probability {
    public static void main(String[] args) {
         ...
         *       이산 균등 분포(2) : 난수 혹은 랜덤수를 말하는데 이 균등성이 깨지면 편향된 결과가 나옴
         *                         편향된 결과가 나오면 세상의 모든 결정,결론들이 의미가 없어진다.
         * 공식 : 위에서 본 기댓값과 같은 공식
         */
        ...
        /**
         * 베르누이 분포
         * 설명 : 어떤 실험/실행이 오직 둘중 하나의 결과만 가질때
         *       그 실험을 '1회'만 할 경우의 확률 분포 결과는 불(bool)값 네,아니요 혹은 참,거짓등
         *       베르누기의 시행(trials) 결과가 예/아니오(이항:binomial)인 어떤 실험을 단 1회만 하는것
         */

        /**
         * 이항 분포 (binomial distribution)
         * 설명 : 이항분포는 베르누이의 시행을 '여러번' 실행하는 경우
         *       거기서 몇 번 성공하는지에 대한 확률 분포
         */
        //이항 확률 변수 : n번 시도에서 확률 p인 결과가 나오는 횟수
        //두개의 주사위를 10번 던질때 7이 나오는 횟수
        //1번 나온다면 > k = 1
        //2번 나온다면 > k = 2 ..
        //10번 나온다면 > k = 10
        ...
        //그럼 10번 던졌을때 1번 7이 나올 확률은?
        //p(x = 1) = 10C1 * 1/6 * 5/6 * 5/6 ... * 5/6
        //첫번째가 더해서 7일확률 나머지 9번째가 아닐확률 그리고 각자리의 조합을 곱한다.
        //2번 7이 나올 확률
        //p(x = 2) = 10C2 * 1/6 * 1/6 * 5/6.. * 5/6
        //..
        //p(x = 10) = 10C10 * 1/6 * 1/6 .. * 1/6
        //즉 n번의 시도에서 k번 성공할 확률은
        //p(x = k) = nCk * p^k * (1 - p)^n - k

        //이항 분포 역시 이산 확률 분포의 한종류니깐 기댓값을 구할수 있음
        //0에서 n번까지 k(확률변수) * nCk * p^k * (1 - p)^n - k(확률 변수의 확률)

        //그냥 이걸 전개하면 최종적으로 np * {확률 변수가 취할수 있는 모든 확률을 더한값}
        //즉 np * {1}, E(X) = np
        //두 주사위의 합이 7일때의 기댓값
        //10번 실험한 경우 E(X) = np = 10 * 1/6 = 1.67...
        //100번 실험한 경우 E(X) = np = 100 * 1/6 = 16.7...
        //실험 횟수가 증가할수록 연속 확률 분포에 가까워진다.
        //기댓값이 성공확률이 가장높다.

        /**
         * 몬테 카를로 알고리듬
         * 설명 : 보통 우리가 알고 있는 알고리듬은 실행하면 언제나 확실한 결과가 나온다.
         *       이걸 결정적 알고리듬이라고 하는데 결정적 알고리듬은 확실한 답을 기다려야 하기때문에 느릴때도 있다.
         *       그래서 확률적 알고리즘중 하나인 몬테 알고리즘을 사용하여 배열 순서대로가 아닌 무작위로 사건을 뽑아서 진행한다.
         */
        //1. 어떤 검사를 정해진 수만큼 수행
        //2. 한번 검사할때마다 검사 대상을 무작위로 뽑음
        //3. 한번 검사할때마다 Yes 혹은 모르겠음이 나옴
        // 모르겠다는건 아직 모든 경우를 다 해본것이 아니기때문에 다시 시도해봐야함.
        //4. 정해진 수만큼 검사한 뒤에도 여전히 모르겠음이면 No라고 결론 내림
        // 하지만 당연히 정답을 찾아내지 못했기 때문에 틀릴 확률이 있음
        // 그렇지만 그 틀릴 확률을 엄청나게 작게 낮추는게 관건 이렇게 하면 100%가 안돼도 결론을 빨리 낼수 있다.

        /**
         * 몬테카를로와 소수 판별
         */
        //소수(prime number) : 자기 자신과 1로만 나머지 없이 나눠지는 수
        //합성수(composite number) : 다른 소수들을 곱해서 만든 숫자
...
//        System.out.println(bPrime);
        ...
        //좀 더 빠른 소수 판별법
        //어떤수를 곱해 n이 나올수 있는 조합은 1 x n부터 √n * √n까지다.
        //√n * √n 이후에 다른 조합을 찾는다면 그건 이미 찾은 조합이다. 무슨말이냐면
        //18를 소수인지 판별하려고 하면
        //1,2,3,6,9,18 약수가 있다. 이 숫자가 쌍일때는 1:18,2:9,3:6이다.
        //18를 루트씌우면 √18 ≈ 4.2426.. 이 나오는데,
        //약수들의 곱으로 봤을때 루트를 씌운값이 중간값(1,2,3,√18,6,9,18)이 되므로
        // 2~√n까지 검색을 한다면 이후의 값은 확인할 필요가 없어진다. 시간복잡도는 O(√N)
        boolean bPrime = true;

        int num = 519561111;
        //루트 함수를 사용하지 않고 j의 제곱값으로 확인
        ...
        System.out.println(num + "이 소수입니까? : " + bPrime);

        //하지만 RSA에 사용하는 소수의 자릿수는
        //10진수로 대략 150~600자리나 된다.
        //반복문을 최소 10^150번이나 돌려야 하고 루트를 씌운다고 해도 10^75번 돌려야한다.

        //이때 바로 몬테 카를로 알고리듬을 쓸수 있는데,
        //몬테 카를로 알고리듬을 쓰려면 소수일 확률이 x퍼센트 라고 알려줘야한다.

        //그것을 알려주는게 바로 밀러라빈 소수 판별법이다.
        //밀러라빈 소수 판별법 : 어떤 홀수 n이 소수인지 확률적으로 알려주는 판별법 (짝수는 필요없다 2로 나눌수 있기 때문)
        //N이 '합성수'이거나, '아마도 소수일 것이다'라고 판단

        //아 도저히 이해 안됨 그냥 넘어감.
        //페르마의 소정리 > 밀러라빈 소수판별법 > 몬테 카를로 알고리듬 이렇게 사용
        //a를 뽑아서 밀러라빈으로 소수일 확률을 구하고,
        //몬테 카를로 알고리듬으로 틀릴 확률을 엄청나게 작게 낮춰서 결론을 빨리 낼수 있다.
        //강의에서는 밀러라빈 정수b 라고 말하는데 블로그에선 a로 말하고 있음
        //모듈러 연산 A mod C > A % C
    }
}
